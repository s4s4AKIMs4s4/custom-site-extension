  ...
  template = (selector, color) => {
    ...
    allEments.forEach((val, idx) => {
      if (
        ...
        val.style.color = `rgba(${color.r}, ${color.g}, ${color.b},${0.7})`;
        return;
      }
...
class Youtube extends commonTemplate {
  ...
  performBackground(color) {
    this.templateBackgroundColor(
      "canvas,#endpoint,.style-scope.ytd-app, caption, center, cite, code,dd, del, dfn, dl, dt, em, embed, fieldset, font, form, h1, h2, h3, h4, h5, h6, hr, i, iframe, img, ins, kbd, label, legend, li, menu, object, ol, p, pre, q, s, samp, small, span, strike, strong, sub, sup, table, tbody, td, tfoot, th, thead, tr, tt, u, ul, var, #items",
      color
    );
    ...
    if (
      document.querySelector(".ytp-exp-bottom-control-flexbox .ytp-volume-area")
    )
      document.querySelector(
    ...
    if (
      document.querySelector(
        "ytd-browse[page-subtype=playlist][has-sidebar_], ytd-browse[page-subtype=show][has-sidebar_]"
      )
    )
      document.querySelector(
        "ytd-browse[page-subtype=playlist][has-sidebar_], ytd-browse[page-subtype=show][has-sidebar_]"
...
const selectObjectForDomen = (currentDomain, state) => {
  //   const domain = rewriteKeys(currentDomain);
  //   console.log('domain')
  //   console.log(domain)
